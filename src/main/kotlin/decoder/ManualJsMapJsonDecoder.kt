...
object ManualJsMapJsonDecoder : Decoder {
  ...
  private val seMap = json(
    ...
    "b" to { content: Json -> ServerPingReplyEvent(content["a"] as Int, content["b"] as Int) },
    "c" to { content: Json -> ServerClipboardEvent(content["a"] as String) },
    "d" to { content: Json -> ServerWindowSetChangedEvent(content["a"].unsafeCast<Array<Json>>().map { it.toWindowData() }) },
    "e" to { content: Json ->
    ...
    "f" to { content: Json -> ServerCaretInfoChangedEvent(content["a"].unsafeCast<Array<Any>>().toCaretInfoChange()) },
    "g" to { content: Json -> ServerMarkdownEvent.ServerMarkdownShowEvent(content["a"] as Int, content["b"] as Boolean) },
    "h" to { content: Json ->
    ...
    "i" to { content: Json -> ServerMarkdownEvent.ServerMarkdownMoveEvent(content["a"] as Int, content["b"].unsafeCast<Json>().toPoint()) },
    "j" to { content: Json -> ServerMarkdownEvent.ServerMarkdownDisposeEvent(content["a"] as Int) },
    "k" to { content: Json -> ServerMarkdownEvent.ServerMarkdownPlaceToWindowEvent(content["a"] as Int, content["b"] as Int) },
    "l" to { content: Json -> ServerMarkdownEvent.ServerMarkdownSetHtmlEvent(content["a"] as Int, content["b"] as String) },
    "m" to { content: Json -> ServerMarkdownEvent.ServerMarkdownSetCssEvent(content["a"] as Int, content["b"] as String) },
    "n" to { content: Json -> ServerMarkdownEvent.ServerMarkdownScrollEvent(content["a"] as Int, content["b"] as Int) },
    "o" to { content: Json -> ServerMarkdownEvent.ServerMarkdownBrowseUriEvent(content["a"] as String) },
    "p" to { content: Json -> ServerWindowColorsEvent() }  // todo: decode map
  )
  ...
  private fun Array<Any>.toEvent(): ServerEvent {
    ...
    return seMap[type].unsafeCast<((Json) -> ServerEvent)?>()?.invoke(content)
      ?: throw IllegalArgumentException("Unsupported event type: ${JSON.stringify(this)}")
  }
  ...
  private val sweMap = json(
    ...
    "g" to { content: Json -> ServerSetFontEvent(content["a"] as Short, content["b"] as Int) },

    "h" to { content: Json -> ServerSetClipEvent(content["a"].unsafeCast<Array<Any>?>()?.toCommonShape()) },

    "i" to { content: Json -> ServerSetStrokeEvent(content["a"].unsafeCast<Array<Any>>().toStrokeData()) },

    "j" to { content: Json -> ServerDrawRenderedImageEvent },

    "k" to { content: Json -> ServerDrawRenderableImageEvent },
    ...
    "o" to { content: Json -> ServerDrawPolylineEvent(content["a"].unsafeCast<Array<Json>>().map { it.toPoint() }) },

    "p" to { content: Json -> ServerSetTransformEvent(content["a"].unsafeCast<Array<Double>>().toList()) },

    "q" to { content: Json ->
      ServerPaintPathEvent(
        (content["a"] as String).toPaintType(),
        content["b"].unsafeCast<Json>().toCommonPath()
      )
    },

    "r" to { content: Json -> ServerSetCompositeEvent(content["a"].unsafeCast<Array<Any>>().toCommonComposite()) },

    "s" to { content: Json -> ServerSetPaintEvent(content["a"].unsafeCast<Array<Any>>().toPaintValue()) },

    "t" to { content: Json -> ServerSetUnknownStrokeEvent(content["a"] as String) }
  )
  ...
  private fun Array<Any>.toWindowEvent(): ServerWindowEvent {
    ...
    return sweMap[type].unsafeCast<((Json) -> ServerWindowEvent)?>()?.invoke(content)
      ?: throw IllegalArgumentException("Unsupported event type: ${JSON.stringify(this)}")
  }
